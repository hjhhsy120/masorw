...
void compute_mem_accumlative_dist(char const *auto_types_name) {
  ...
  while (slope_pointer < slope_cnt) {
    ...
    if ((change_type == ONLINE2REJECT && node_type_tmp[node] == USEONLINE) || 
        ...
        fprintf(fp, "%d %lld %lld %d %d\n", slope_pointer, space_inc, space_now, node, change_type);
    }
    slope_pointer++;
  }
  fclose(fp);
...
void update_memory(float cost_init_time) {
  ...
  float greedy_cost = chrono::duration_cast<chrono::duration<float>>(end1 - begin1).count();
  if (verbosity > 0)
...
#pragma omp parallel for schedule(dynamic)
  for (int i = prev_slope_pointer; i < slope_pointer; i++)
    slopes[i].real_use_slope();
#pragma omp parallel for schedule(dynamic)
  for (int i = prev_slope_pointer - 1; i >= slope_pointer; i--)
    slopes[i].real_cancel_slope();
...
#pragma omp parallel for schedule(dynamic)
  ...
  float node_sampler_init = chrono::duration_cast<chrono::duration<float>>(end2 - begin2).count();
  ...
  if (verbosity > 0) {
    ...
    if (full_slope_pointer == slope_cnt)
      cout << "All slopes used" << endl;
    else
      cout << "First skip at node " << slopes[full_slope_pointer].node
           << ", degree: " << degrees[slopes[full_slope_pointer].node]
           << ", change type: " << (int)slopes[full_slope_pointer].change_type
           << ", need space: " << slopes[full_slope_pointer].space_inc
           << ", slope: " << slopes[full_slope_pointer].val
           << endl;
  }
  cout << "#LOG_AUTO: "
       << cost_init_time
...
inline void init_auto() {
  ...
  cout << "After first initialization in init_auto" <<endl;
  showMemoryInfo();
  ...
  auto end1 = chrono::steady_clock::now();
  float cost_init_time = chrono::duration_cast<chrono::duration<float>>(end1 - begin1).count();
  if (verbosity > 0)
  ...
  for (int node = 0; node < nv; node++) {
    node_type[node] = USEONLINE;
    //LL alias_space = (LL)degrees[node] * (degrees[node] + 1)
     //               * (sizeof(int) + sizeof(float));
    LL alias_space = (LL)degrees[node] * (degrees[node] + 1)
                    * (sizeof(int) + sizeof(float));
    tot_mem += alias_space;
        // here + 1 is sample_start_edge
    LL reject_space = degrees[node] * (sizeof(int) + sizeof(float) + sizeof(float));

    float alias_time = 2;
    float reject_time = (2 + log(degrees[node]) / log(2)) * rej_time[node];
    float online_time = (2 + log(degrees[node]) / log(2)) * degrees[node];
    ...
    if (another_cost) {
      if (alias_time <= online_time)
        slopes[slope_cnt++].set_slope(degrees[node] * slope_online_alias, node, alias_space, ONLINE2ALIAS);
      if (reject_time < online_time)
        slopes[slope_cnt++].set_slope(degrees[node] * slope_online_reject, node, reject_space, ONLINE2REJECT);
      if (alias_time <= reject_time)
        slopes[slope_cnt++].set_slope(degrees[node] * slope_reject_alias, node, alias_space - reject_space, REJECT2ALIAS);
    } else {
      if (alias_time <= online_time)
...
#pragma omp parallel for
  for (int node = 0; node < nv; node++) {
    if (node_type[node] == USEREJECT)
      one_node_reject(node);
    else if (node_type[node] == USEALIAS) {
      one_node_alias_reject(node);
      one_node_edge_alias(node);
    }
  }
  ...
  if (verbosity > 0)
    cout << "Finished initialization, using "
         << chrono::duration_cast<chrono::duration<float>>(end3 - begin3).count()
         << " s to run" << endl
         << "Need " << (double)space_now / 1024 / 1024
         << " Mb memory" << endl;
*/
}
...
inline int sample_start_edge_auto(int n0, myrandom &r, int tid) {
  if (node_type[n0] == USEONLINE)
    return sample_start_edge_online(n0, r, tid);
  return sample_start_edge(n0, r);
}

inline int sample_edge_auto(int n0, int lastedgeidx, myrandom &r, int tid) {
