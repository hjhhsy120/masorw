...
//  if (dstadj == src)
//    return weights[dstadji] / p;
//  else if (has_edge(dstadj, src))
//    return weights[dstadji];
//  else
//    return weights[dstadji] / q;
if (find_edge(dstadj, src) == -1)
    return (dstadj == src ? weights[dstadji] / p : weights[dstadji] / q);
  else
    return weights[dstadji];
}
...
int main(int argc, char **argv) {
  ...
  if ((a = ArgPos(const_cast<char *>("-memorylist"), argc, argv)) > 0)
    memory_pointer = a + 1;
  ...
  if ((a = ArgPos(const_cast<char *>("-memorychange"), argc, argv)) > 0) {
    ...
    cout <<"Execute Dynamic Memory Budget: " << logfile << endl;
    testUpdate(logfile);
    return 0;
  }
