...
void init_reverse() {
  edges_r =
      static_cast<LL *>(malloc(ne * sizeof(LL)));
  ...
  for (int src = 0; src < nv; src++) {
    for (int lastedgeidx = offsets[src]; lastedgeidx < offsets[src + 1]; lastedgeidx++) {
      ...
      if (degrees[src] < degrees[dst] || (degrees[src] == degrees[dst] && src < dst))
        continue;
      LL pos = find_edge(dst, src); // find edge from dst to src
...
int init_graph(string network_file) {
  ...
  if (inputFile.is_open()) {
    ...
    offsets = static_cast<int *>(
        malloc((nv + 1) * sizeof(int32_t)));
    edges =
        static_cast<int *>(malloc(ne * sizeof(int32_t)));
    weights =
        static_cast<float *>(malloc(ne * sizeof(int32_t)));
    inputFile.read(reinterpret_cast<char *>(offsets), nv * sizeof(int32_t));
    offsets[nv] = static_cast<int>(ne);
    inputFile.read(reinterpret_cast<char *>(edges), sizeof(int32_t) * ne);
    inputFile.read(reinterpret_cast<char *>(weights), sizeof(float) * ne);
    if (verbosity > 0)
      cout << "nv: " << nv << ", ne: " << ne << endl;
    inputFile.close();
    degrees =
        static_cast<int *>(malloc(nv * sizeof(int32_t)));
    for (int i = 0; i < nv; i++)
